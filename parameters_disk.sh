...
device_model=$(sudo /usr/sbin/smartctl -a /dev/sda | grep 'Device Model' | cut -f2 -d ":")
serial_number=$(sudo /usr/sbin/smartctl -a /dev/sda | grep 'Serial Number' | cut -f2 -d ":")
disk_capacity=$(sudo /usr/sbin/smartctl -a /dev/sda | grep 'User Capacity' | cut -f2 -d ":" | cut -f2 -d "[" | cut -f1 -d "]")
...
do 
  ...
  value=$(sudo smartctl -a /dev/"$1" | grep "$2" | awk '{print $$10}') # DEVOLVE VALOR DO ELEMENTO
